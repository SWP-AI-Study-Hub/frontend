    ...
    <main id="main-content" className="library-page">
      <header className="library-page-heading">
        <div>
          ...
          <p>Search metadata and content, inspect indexing status, or continue with AI.</p>
        </div>
      ...
      <div className="library-result-count">
        ...
        <span>{documents.filter((document) => document.indexStatus === 'READY').length} AI ready</span>
      </div>
                ...
                <tr key={document.id}>
                  ...
                  <td>
                    <span className={`index-status index-status--${document.indexStatus.toLowerCase()}`}>
                      {document.indexStatus === 'READY' ? 'AI ready' : document.indexStatus.toLowerCase()}
                    </span>
                  </td>
                  <td>
                    <div className="document-actions">
                      <button type="button" title="Preview" onClick={() => setPreviewDocument(document)}>
                        <Eye size={16} />
                      </button>
                      <button type="button" title="Download" onClick={() => downloadDemoDocument(document)}>
                        <Download size={16} />
                      </button>
                      <Link href="/ask-document" className="ask-document-action">
                        <Bot size={16} />
                        Ask AI
                      </Link>
                    </div>
        ...
        <div className="preview-overlay" role="presentation" onMouseDown={() => setPreviewDocument(undefined)}>
          ...
          >
            <header>
              ...
              <button type="button" className="icon-button" onClick={() => setPreviewDocument(undefined)}>
                <X size={18} />
              </button>
            </header>
            <div className="preview-document-sheet">
              ...
              <blockquote>
                AI preview is ready. Open Ask This Document to explore this source with grounded citations.
              </blockquote>
            </div>
            <footer>
              <button type="button" className="secondary-button" onClick={() => downloadDemoDocument(previewDocument)}>
                <Download size={16} />
                Download
              </button>
              <Link href="/ask-document" className="primary-button">
                <Bot size={16} />
                Ask AI
              </Link>
            </footer>
